...
//        GridSize(100),
        ...
        ExperimentStartYear(100), ExperimentStartWeek(1),
        ExperimentDuration(0), EutrophicationIntensity(0),
        AbvHerbExclusion(0), BelHerbExclusion(0),
        Aampl(0), Bampl(0),
        SeedInput(0), SeedRainType(0),
        averageMonocultureRepro(8188)
{
    // averageRepro_perRecruit is measured on a 1 m2 gridspace, so it must be scaled to the grid's extent
    averageMonocultureRepro = averageMonocultureRepro * (pow(GridSize, 2) / pow(100, 2));
}

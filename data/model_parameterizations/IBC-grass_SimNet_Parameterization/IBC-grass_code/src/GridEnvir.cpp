...
void GridEnvir::InitInds()
{
    ...
    else if (Parameters::parameters.mode == SimNet)
    {
        for (auto const& it : Traits::pftTraitTemplates)
        {
            double a = static_cast<double>(Parameters::parameters.averageMonocultureRepro) / static_cast<double>(Traits::pftTraitTemplates.size());
            double b = static_cast<double>(it.second->seedMass);
            int n = static_cast<int>(std::floor( a / b ));

            InitSeeds(it.first, n, estab);
...
void GridEnvir::SeedRain()
{
    ...
    for (auto const& it : Traits::pftTraitTemplates)
    {
        ...
        switch (Parameters::parameters.SeedRainType)
        {
            ...
            case 2:
            {
                // (SeedRain/100) * (averageMonocultureInteralSeedRain / Ninitial)
                double a = static_cast<double>(Parameters::parameters.SeedInput) / 100; // SeedInput is a percentage
                double b = static_cast<double>(Parameters::parameters.averageMonocultureRepro) / static_cast<double>(Traits::pftTraitTemplates.size());
                double c = static_cast<double>(it.second->seedMass);
                n = static_cast<int>(std::floor( (a * b) / c) );

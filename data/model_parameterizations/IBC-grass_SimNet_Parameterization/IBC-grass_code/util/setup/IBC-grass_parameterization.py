...
community_out   = 0 # Print output about the environment, etc.  (0) No; (1) Yes
...
IC_vers          = [0, 1] # IBC-grass run mode -- Negative frequency dependence
MODE             = [4] # (0) Community Assembly; (1) Invasion criterion; (2) Catastrophic disturbance; (3) Eutrophication scenario; (4) SimNet
N_PFTs           = [1, 2, 4, 8, 16, 32, 64] # UNUSED WITH PAIRWISE INVASION CRITERION
ITVsd            = [0]
Tmax             = [200]
...
def buildBatchScripts(SimFile):
    ...
    sim_files = []
    for core in range(1, N_SLOTS+1): # one new SimFile for each core
        i = 0
...
def buildPFTs():
    ...
    for N_PFT in N_PFTs:  
        ...
        for s in range(1, N_COMS+1): # one SimNr per sample of PFTypes. 
            
            community = [] 
            if (PFT_type == "SIM_NET" and N_PFT == 1): # For SimNet, ensure that all the monocultures are run
                assert(N_COMS == 64)
                community.append(PFTs[s-1])
            else:
            ...
            for base_param in itertools.product(*base_params):
                ...
                SimFile.append(" ".join([str(SimNr), str(ComNr), base_param.toString(), \
                    str(weekly), str(individual_out), str(population_out), \
                    str(populationSurvival_out), str(trait_out), str(community_out), Com_FN, "\n"]))
...
def buildPairs():
    ...
    for pair in pairs:
        ...
        for base_param in itertools.product(*base_params):
            ...
            SimFile.append(" ".join([str(SimNr), str(ComNr), base_param.toString(), \
                str(weekly), str(individual_out), str(population_out), str(populationSurvival_out), \
                str(trait_out), str(community_out), PFT_FN, "\n"]))

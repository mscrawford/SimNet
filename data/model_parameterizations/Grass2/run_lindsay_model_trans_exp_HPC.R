...
write.csv(data.frame(SpeciesID=1:64, traits_out0), "lindsaymod_trans_exp_speciesdata.csv", row.names=F)
...
for(ii in 1:nrow(simmat)) {
  ...
  if(ii/10==floor(ii/10)) {
    ...
    trtable<-save(list = c("datout"), file = "lindsaymod_trans_exp_out_HPC.rda")
  }
}
trtable<-save(list = c("datout"), file = "lindsaymod_trans_exp_out_HPC.rda")

---
title: "Functional dispersion in TROLL"
output: 
    html_document:
        toc: true
        toc_float: true
...
TROLL <- readRDS(file = "biomass_brms_models.rds")%>%
    filter(Model == "TROLL") %>% 
    filter(measure == "FDis")

```
This document describes TROLL, using **functional dispersion** as the focal biodiversity metric.
...
```{r plotted-data}
ggplot(TROLL %>% unnest(data)) +
    geom_point(aes(x = FDis,
                   y = total.biomass,
                   color = as.factor(Ninitial))) +
    facet_grid(. ~ Stage) +
    labs(x = "Realized functional dispersion",
         y = "Total biomass",
...
The main effect represents the relationship between realized diversity and total biomass without taking into
account the planted species richness treatment. 
...
rstantools::bayes_R2(TROLL$mainEffect[[1]])
...
brms::pp_check(TROLL$mainEffect[[1]]) +
...
sjPlot::plot_model(TROLL$mainEffect[[1]],
...
rstantools::bayes_R2(TROLL$treatmentEffect[[1]])
...
brms::pp_check(TROLL$treatmentEffect[[1]]) +
    labs(x = "Total biomass") +
    theme_bw()
...
sjPlot::plot_model(TROLL$treatmentEffect[[1]],
                   title = "89% Highest-density intervals")
...
data <- TROLL %>% unnest(data)
mainEffect_ <- TROLL$mainEffect[[1]]
treatmentEffect_ <- TROLL$treatmentEffect[[1]]

---
title: "Functional dispersion in adam"
output: 
    html_document:
        toc: true
        toc_float: true
...
adam <- readRDS(file = "brms_models.rds") %>%
    filter(Model == "adam") %>% 
    filter(measure == "FDis")

```
This document describes adam, using **functional dispersion** as the focal biodiversity metric.
...
rstantools::bayes_R2(adam$mainEffect[[1]])
...
brms::pp_check(adam$mainEffect[[1]]) +
...
sjPlot::plot_model(adam$mainEffect[[1]],
...
rstantools::bayes_R2(adam$treatmentEffect[[1]])
...
brms::pp_check(adam$treatmentEffect[[1]]) +
    labs(x = "Total biomass") +
    theme_bw()
...
sjPlot::plot_model(adam$treatmentEffect[[1]],
                   title = "89% Highest-density intervals")
...
data <- adam %>% unnest(data)
mainEffect_ <- adam$mainEffect[[1]]
treatmentEffect_ <- adam$treatmentEffect[[1]]

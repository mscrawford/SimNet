---
title: "Richness in IBC_grass.noNDD"
output: 
    html_document:
        toc: true
        toc_float: true
...
```
This document describes IBC_grass.noNDD, using **Richness** as the focal biodiversity metric.
...
brm(formula = total.biomass ~ -1 + Stage + Stage:Richness,
...
brm(formula = total.biomass ~ -1 + Ninitial:Stage + Ninitial:Stage:Richness,
    data = IBC_grass.noNDD)
...
describes this this combination with reference to `Richness`.
...
mainEffect <- data %>%
    ...
    inner_join(extract_significances_mainEffect(mainEffect_, "Richness"))
...
treatmentEffect <- data %>%
    ...
    inner_join(extract_significances_treatmentEffect(treatmentEffect_, "Richness"))

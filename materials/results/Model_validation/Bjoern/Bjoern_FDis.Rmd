---
title: "Functional dispersion in bjoern"
output: 
    html_document:
        toc: true
        toc_float: true
...
bjoern <- readRDS(file = "brms_models.rds") %>%
    filter(Model == "bjoern") %>% 
    filter(measure == "FDis")

```
This document describes bjoern, using **functional dispersion** as the focal biodiversity metric.
...
rstantools::bayes_R2(bjoern$mainEffect[[1]])
...
brms::pp_check(bjoern$mainEffect[[1]]) +
...
sjPlot::plot_model(bjoern$mainEffect[[1]],
...
rstantools::bayes_R2(bjoern$treatmentEffect[[1]])
...
brms::pp_check(bjoern$treatmentEffect[[1]]) +
    labs(x = "Total biomass") +
    theme_bw()
...
sjPlot::plot_model(bjoern$treatmentEffect[[1]],
                   title = "89% Highest-density intervals")
...
data <- bjoern %>% unnest(data)
mainEffect_ <- bjoern$mainEffect[[1]]
treatmentEffect_ <- bjoern$treatmentEffect[[1]]

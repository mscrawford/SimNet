---
title: "Functional dispersion in PPA"
output: 
    html_document:
        toc: true
        toc_float: true
...
PPA <- readRDS(file = "brms_models.rds") %>%
    filter(Model == "PPA") %>% 
    filter(measure == "FDis")

```
This document describes PPA, using **functional dispersion** as the focal biodiversity metric.
...
rstantools::bayes_R2(PPA$mainEffect[[1]])
...
brms::pp_check(PPA$mainEffect[[1]]) +
...
sjPlot::plot_model(PPA$mainEffect[[1]],
...
rstantools::bayes_R2(PPA$treatmentEffect[[1]])
...
brms::pp_check(PPA$treatmentEffect[[1]]) +
    labs(x = "Total biomass") +
    theme_bw()
...
sjPlot::plot_model(PPA$treatmentEffect[[1]],
                   title = "89% Highest-density intervals")
...
data <- PPA %>% unnest(data)
mainEffect_ <- PPA$mainEffect[[1]]
treatmentEffect_ <- PPA$treatmentEffect[[1]]
...
cowplot::save_plot(p, filename = "~/Desktop/FDis.pdf", ncol = 2, base_asp = 1.1)
